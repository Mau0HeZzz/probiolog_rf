...
$db_list = CIBlockSection::GetList(Array('SORT'=>'ASC'), $arFilter, true, array("ID","NAME","DESCRIPTION","UF_*"));
$i = 0;

while($ar_result = $db_list->GetNext()) {
    if($ar_result['ELEMENT_CNT']) {
        // Записываем инфо выбранных разделов для&nbsp;дальнейшего использования
        $arSection[$i] = array(
            'ID' => $ar_result['ID'],
            'NAME' => $ar_result['NAME'],
...
<? foreach($arSection as $arSectionInfo): ?>
    <div class="buy__online buy__pharmacy">
        <?=$arSectionInfo['NAME']?>
    </div>
    <div class="buy__items">
        <? foreach($arResult["ITEMS"] as $arItem): ?>
            <? if ($arItem['IBLOCK_SECTION_ID'] == $arSectionInfo['ID']): ?>
